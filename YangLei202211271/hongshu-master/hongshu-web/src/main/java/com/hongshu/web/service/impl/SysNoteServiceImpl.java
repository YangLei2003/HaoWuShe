...
public class SysNoteServiceImpl implements ISysNoteService {
    ...
    public List<WebNote> selectNoteList(Query query) {
        ...
        // 用 author 字段查作者
        qw.lambda().like(ValidatorUtil.isNotNull(query.get("author")), WebNote::getAuthor, query.get("author"));
    ...
    public int insertNote(WebNote note, MultipartFile file) {
        ...
        String userName = sysUserMapper.getUserName();
...
//        }
    ...
    public boolean auditNote(String noteId, String auditType) {
        ...
        if ("pass".equals(auditType)) {
            ...
            Integer likeCount = likeOrCollectMapper.getLikeCount(note.getId());
            Integer count = likeOrCollectMapper.getById(noteId, Integer.valueOf(note.getUid()));
            noteSearchVo.setUsername(user.getUsername())
                    .setAvatar(user.getAvatar())
                    .setLikeCount(Long.valueOf(likeCount))
                    .setCategoryParentName(parentCategory.getTitle())
                    .setTime(System.currentTimeMillis());
            if (count > 0) {
                noteSearchVo.setIsLike(true);
            } else {
