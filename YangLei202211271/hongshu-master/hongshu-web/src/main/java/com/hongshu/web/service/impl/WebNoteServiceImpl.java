...
public class WebNoteServiceImpl extends ServiceImpl<WebNoteMapper, WebNote> implements IWebNoteService {
    ...
    @Autowired
    private IWebFollowService followerService;
    @Autowired
    ...
    public NoteVO getNoteById(String noteId) {
        ...
        Product product = productMapper.getById(note.getProductId());
        ...
        if(product!=null){
                    noteVo.setProductName(product.getProductName())
                    .setProductPrice(String.valueOf(product.getPrice()))
        ...
        List<WebLikeOrCollect> likeOrCollectionList = likeOrCollectionService.list(new QueryWrapper<WebLikeOrCollect>()
                .eq("like_or_collection_id", noteId).eq("uid", currentUid));
        ...
        //得到标签
        List<WebTagNoteRelation> tagNoteRelationList = tagNoteRelationService.list(new QueryWrapper<WebTagNoteRelation>()
                .eq("nid", noteId));
