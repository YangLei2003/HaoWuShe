...
public class WebLikeOrCollectServiceImpl extends ServiceImpl<WebLikeOrCollectMapper, WebLikeOrCollect> implements IWebLikeOrCollectService {
    ...
    @Autowired
    private WebLikeOrCollectMapper webLikeOrCollectMapper;
    ...
    public void likeOrCollectionByDTO(LikeOrCollectDTO likeOrCollectDTO) {
        ...
        if (isLikeOrCollection(likeOrCollectDTO)) {
            this.remove(new QueryWrapper<WebLikeOrCollect>().eq("uid", currentUid)
                    .eq("like_or_collection_id", likeOrCollectDTO.getLikeOrCollectionId())
                    .eq("type", likeOrCollectDTO.getType()));
    ...
    private void updateNoteToES(WebNote note) {
        try {
            // 先查询 ES 中的文档
            String userId = AuthContextHolder.getUserId();
            GetResponse<NoteSearchVO> getResponse = elasticsearchClient.get(g -> g
                    .index(NoteConstant.NOTE_INDEX)
                    .id(note.getId()), NoteSearchVO.class);

            if (getResponse.found()) {
                NoteSearchVO existing = getResponse.source();
                if (existing != null) {
                    // 更新点赞数
                    existing.setLikeCount(note.getLikeCount());
                    Integer count = webLikeOrCollectMapper.getById(note.getId(),Integer.valueOf(userId));
                    if (count > 0) {
