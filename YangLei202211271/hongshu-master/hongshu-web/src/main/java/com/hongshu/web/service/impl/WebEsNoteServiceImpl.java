...
public class WebEsNoteServiceImpl extends ServiceImpl<WebNoteMapper, WebNote> implements IWebEsNoteService {
    ...
    public Page<NoteSearchVO> getNoteByDTO(long currentPage, long pageSize, EsNoteDTO esNoteDTO) {
        ...
        try {//告诉ES:我要在note_index这个索引里进行操作
            ...
            if (StringUtils.isNotBlank(esNoteDTO.getKeyword())) {
                builder.query(q -> q.bool(b -> b//实际上构建了应该Json查询,相当于:
//                {
//                 "query": {
//                    "bool": {
//                        "should": [
//                        { "match": { "title": { "query": "Java", "boost": 1 } } },
//                        { "match": { "username": { "query": "Java", "boost": 0.5 } } },.....
//                    ]}
//                }
//                }       query查询操作  bool逻辑组合(相当于where)  should应该匹配(类似OR):满足的条件越多,得分越高  boost:权重
            ...
            //以上都属于组合逻辑条件.把构建器转成真正的对象
            SearchRequest searchRequest = builder.build();
            //由es客户端执行搜索,并把结果转成NoteSearchVO对象
