...
public class FileServiceImpl extends SuperServiceImpl<SysFileMapper, WebFile> implements IWebFileService {
    @Value("${hongshu.profile}") // 设置默认值防止报错
    ...
    public String uploadFile(MultipartFile file) {
        ...
        try {
            ...
            webFile.setFileOldName(originalFilename);
            webFile.setPicName(fileName);
            webFile.setPicExpandedName(suffix.replace(".", ""));
            webFile.setFileSize(file.getSize());
            webFile.setPicUrl("/profile/" + fileName);
            webFile.setCreateTime(new Date());

            // 4. 【核心改进】稳健获取 UserUid，绝不抛出 ServiceException
            String finalUserId = "1"; // 默认给个管理员ID或者匿名ID备用
            try {
                // 尝试从 Security 上下文拿 (如果 Token 没问题，这行能成)
                Object principal = SecurityUtils.getAuthentication().getPrincipal();
                if (principal instanceof LoginUser) {
                    finalUserId = ((LoginUser) principal).getUserId().toString();
                } else {
                    // 如果上下文拿不到，说明 Token 校验被拦截器挡了，我们去 Request 里手动抠
                    HttpServletRequest request = RequestHolder.getRequest();
                    // 尝试从你项目常用的属性名里拿
                    Object reqUserId = request.getAttribute(SysConf.USER_UID);
                    if (reqUserId != null) {
                        finalUserId = reqUserId.toString();
                    }
                }
            } catch (Exception e) {
                // 就算全失败了，也只是 log 一下，绝对不让程序崩掉
                log.warn("无法从安全上下文获取UID，使用默认ID: 1");
            }

            webFile.setUserUid(finalUserId);

            // 5. 保存到数据库
            this.save(webFile);

            return webFile.getPicUrl();

        } catch (IOException e) {
            log.error("文件上传物理写入失败: {}", e.getMessage());
            return "";
        } catch (Exception e) {
            log.error("文件上传业务处理异常: {}", e.getMessage());
            return "";
        }
    }
}
