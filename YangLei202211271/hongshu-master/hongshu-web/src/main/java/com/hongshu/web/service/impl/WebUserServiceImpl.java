...
public class WebUserServiceImpl extends ServiceImpl<WebUserMapper, WebUser> implements IWebUserService {
    ...
    public Page<NoteSearchVO> getTrendByUser(long currentPage, long pageSize, String userId, Integer type) {
        ...
        if (type == 1) {
            ...
            return this.getProductPageByUser(currentPage, pageSize, userId);
        } else {
    ...
    private Page<NoteSearchVO> getLikeOrCollectionPageByUser(long currentPage, long pageSize, String userId, Integer type) {
        ...
        for (WebLikeOrCollect model : likeOrCollectionList) {
            ...
            if (noteSearchVo != null && user != null) {
...
//    @DataScope(deptAlias = "d")
    ...
    private Page<NoteSearchVO> getLikeOrCollectionPageByUser(long currentPage, long pageSize, String userId) {
        ...
        notePage = noteMapper.selectPage(new Page<>(currentPage, pageSize), new QueryWrapper<WebNote>()
                .eq("uid", userId)
    ...
    private Page<NoteSearchVO> getProductPageByUser(long currentPage, long pageSize, String userId) {
        Page<NoteSearchVO> noteSearchVoPage = new Page<>();

        // 1. 分页查询商品原始数据
        Page<Product> productPage = productMapper.getByUserId(new Page<>(currentPage, pageSize), userId);
        ...
        for (Product product : productList) {
            NoteSearchVO vo = new NoteSearchVO()
                    .setId(String.valueOf(product.getId())) // Integer 转 String
                    .setTitle(product.getProductName())      // 商品名 -> 标题
                    .setNoteCover(product.getCover())        // 封面 -> 瀑布流图
                    .setUid(String.valueOf(product.getSellerId()))
                    .setNoteType("1"); // 保持笔记卡片布局

            // 填充用户信息，供 Note.vue 渲染头像和昵称
            if (user != null) {
                vo.setUsername(user.getUsername())
                        .setAvatar(user.getAvatar());
            }

            // 时间转换：LocalDateTime 转为毫秒时间戳
            if (product.getCreateTime() != null) {
                vo.setTime(product.getCreateTime().atZone(ZoneId.systemDefault()).toInstant().toEpochMilli());
            }

            voList.add(vo);
        }

        noteSearchVoPage.setRecords(voList);
        noteSearchVoPage.setTotal(productPage.getTotal());
        return noteSearchVoPage;
    }
}
