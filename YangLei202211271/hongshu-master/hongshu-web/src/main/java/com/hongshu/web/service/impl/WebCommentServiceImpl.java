...
public class WebCommentServiceImpl extends ServiceImpl<WebCommentMapper, WebComment> implements IWebCommentService {
    ...
    public CommentVO saveCommentByDTO(CommentDTO commentDTO) {
        ...
        // 2. 【新增】写入正式评论表 (WebComment)
        // 如果你希望回复评论也是立即持久化并刷新的话，这里必须存入主表
        WebComment webComment = ConvertUtils.sourceToTarget(commentsync, WebComment.class);
        Integer exist = commentMapper.exist(commentsync.getId());
        if(exist!=null ){// 使用 MyBatis-Plus 的 save 方法写入 web_comment 表
            System.out.println("评论已存在,跳过插入");
        }else{
            this.save(webComment);
        }
        // 3. 更新笔记评论计数
        ...
        if (!"0".equals(commentDTO.getPid())) {
            ...
            // 再查同步表（保持数据一致性）
            WebCommentSync parentSync = commentSyncMapper.selectById(commentDTO.getPid());
            if (parentSync != null) {
                parentSync.setTwoCommentCount(parentSync.getTwoCommentCount() + 1);
                commentSyncMapper.updateById(parentSync);
            }
        }

        // 5. 【可选】如果是回复评论，发送消息通知
        if (!"0".equals(commentDTO.getPid())) {
            chatUtils.sendMessage(commentDTO.getReplyUid(), 1);
        }
    ...
    public void syncCommentByIds(List<String> commentIds) {
        ...
        if (!commentIds.isEmpty()) {
            ...
            // 过滤出不存在的评论
            List<WebComment> toInsert = new ArrayList<>();
            for (WebComment comment : comments) {
                Integer exist = commentMapper.exist(comment.getId());
                if (exist == null) {  // 不存在才插入
                    toInsert.add(comment);
                } else {
                    System.out.println("评论已存在,跳过插入, id=" + comment.getId());
                }
            }
            // 批量插入
            if (!toInsert.isEmpty()) {
    ...
    public void deleteCommentById(String commentId) {
        ...
        // 2. 删除同步表
        commentSyncMapper.deleteById(commentId);

        // 3. (可选) 减少笔记表中的 commentCount 计数
        // ...
    }
}
